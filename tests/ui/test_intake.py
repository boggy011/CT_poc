...
def test_valid_submit_shows_reference_and_persists(app_for: Callable[[str], AppTest]):
    ...
    at.button(key=SUBMIT_KEY).click().run()
    assert not at.exception
    assert at.success and "received" in at.success[0].value
    ...
    assert newest.submission_id[-12:].upper() in " ".join(m.value for m in at.metric)
...
def test_submitted_result_does_not_leak_to_next_user(app_for: Callable[[str], AppTest]):
    ...
    assert not at.exception
    rendered = " ".join(str(e.value) for e in list(at.markdown) + list(at.caption) + list(at.success) + list(at.metric))
