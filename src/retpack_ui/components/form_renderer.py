...
def render_field(f: FieldSpec, choices: Mapping[str, tuple[Choice, ...]], *, key_prefix: str = KEY_PREFIX) -> Any:
    ...
    if f.type is FieldType.STRING:
        return st.text_input(label, key=key, max_chars=f.max_length, help=f.help)
    if f.type is FieldType.TEXT:
        return st.text_area(label, key=key, max_chars=f.max_length, help=f.help)
    if f.type is FieldType.INTEGER:
        lo = int(f.min) if f.min is not None else None
        hi = int(f.max) if f.max is not None else None
    ...
    if f.type is FieldType.DECIMAL:
        ...
        return st.number_input(label, key=key, value=None, step=0.01, format="%.2f", min_value=lo_f, max_value=hi_f, help=f.help)
