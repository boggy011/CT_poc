...
**Design pass (3 Sep 2026, later).** Theme through `.streamlit/config.toml` (slate neutrals, copper accent, Barlow Semi Condensed headings, Public Sans body, IBM Plex Mono for codes, dark sidebar), placeholder logo assets, sidebar rebuilt as a menu with icons and a signed-in block, page headers, sectioned cards, status badges and human status labels, Title Case labels. The screens package was renamed from `pages/` to `screens/` because Streamlit treats a `pages/` folder as its multipage convention and injected its own navigation. Raw HTML is confined to `theme.py` and is static; a test enforces both. Screenshots in `docs/screenshots/`. **Demo impersonation** (`RETPACK_DEMO_IMPERSONATION=1`, dev only): a token-verified internal user may view the portal as any provisioned demo user; customers never can; each switch is audited. The `AccountDirectory` port gained `list_emails()` for the switcher.

### Phase B — Real adapters (needs inputs 3, 4, 6, 7)

**Status (3 Sep 2026, later): built and run against the dev workspace.** Catalog `ct_retpack_dev` created, migrations applied, reference stubs and demo data seeded, the contract and isolation suites passed on real Delta and the real Volume (29 cases), and the app is deployed and healthy at https://retpack-portal-1283361390446220.0.azure.databricksapps.com. Remaining Phase B exit items: a signed-in browser check of the identity flow, the data-contract sign-off (input 3), and the real reference view DDL (input 4). Earlier note kept for history: Every step below is implemented with the workspace-specific pieces behind small interfaces: the shared SQL repositories run the full contract and isolation suites on SQLite on every commit, and the Delta and Lakebase executors, Volume files client and token identity lookup are unit-tested against fakes. What remains for Phase B exit is purely execution against a workspace once input 7 arrives: apply the migrations, run `RETPACK_TEST_DELTA=1 pytest tests/contract tests/isolation`, deploy the app, and confirm the assumed reference view DDL (input 4). Additions made while building: a fourth backend, `sqlite`, gives a persistent local demo through the same SQL code path (`make run-sqlite`); `docs/data_contract.md` v0.9 and `docs/deploy.md` are written; the asset bundle in `databricks.yml` is unvalidated (validation needs credentials). Presigned download URLs (B4) are not available for Volumes, so the two-step download from Phase A stays.
